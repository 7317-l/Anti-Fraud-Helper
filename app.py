...
.popup-box {background: linear-gradient(135deg, #ff4444 0%, #cc0000 100%); color: white; padding: 40px 50px; border-radius: 20px; box-shadow: 0 0 60px rgba(255,0,0,0.7); width: 520px; text-align: center; border: 5px solid #ffcc00; animation: popIn 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275);}
...
.close-popup-btn {background: rgba(255,255,255,0.9); color: #cc0000; border: none; padding: 12px 30px; border-radius: 25px; font-size: 18px; font-weight: bold; cursor: pointer; margin-top: 20px; transition: all 0.3s ease;}
.close-popup-btn:hover {background: white; transform: scale(1.05);}
...
def show_popup(message, duration=5):
    ...
    st.session_state.popup_remaining = duration
    st.rerun()
...
def render_popup():
    ...
    <div class="popup-overlay" id="popup-overlay">
        <div class="popup-box">
            ...
            <button class="close-popup-btn" onclick="closePopup()">✕ 我知道了，关闭弹窗</button>
            <div class="countdown-badge" id="countdown">⏱️ 窗口将在 {st.session_state.popup_remaining} 秒后自动关闭</div>
        </div>
    </div>
    <script>
...
# --- 广告轮播（不用st.rerun，只用st.empty占位，手动切换） ---
ad_messages = [
...
if prompt or uploaded_img or uploaded_audio:
    ...
    with st.chat_message("assistant"):
        ...
        if risk_level == "高风险":
            ...
            if popup_msg:
                show_popup(popup_msg, duration=5)
        elif risk_level == "中风险":
...
# --- 监听前端JS发送的关闭弹窗消息 ---
if st.session_state.get("js_close_popup"):
    hide_popup()
    st.session_state.js_close_popup = False